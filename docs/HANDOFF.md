...
- **현재 버전**: `v1.0.1` (GitHub 배포 및 태그 푸시 완료)
- **진행 상황**: 카드 z-index 정규화(Renormalization) 및 레이어 우선순위 버그 수정(4차 작업) 완벽 검증 완료.
- **다음 목표**: `TODO.md`에 명시된 **5차 작업 시스템 고도화 (Phase 1 & Phase 2)** 착수 준비.
...
- **`사용법.txt`가 v1.0.0·옛 3분할 UI 기준으로 뒤처져 있음.** v1.0.1에서 카드 워크스페이스로 크게 바뀌었으나 사용법은 미갱신. **다음 릴리스 전에 카드 시스템 기준으로 다시 쓸 것**(카드 추가/이동/리사이즈/스냅/잠금/역본 칩/사전 연결 등).
