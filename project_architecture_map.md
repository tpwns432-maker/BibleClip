...
- **현재 버전**: v1.1.6 (`bibleclip/_version.py` = `__version__`, ASCII-only single source of truth)
- **killswitch**: `recommend_version` = 1.1.5 (직전 버전 규칙)
...
- **마지막 맵 동기화**: 2026-06-17 (v1.1.6 본문 보기 모드/병렬 독서 컬럼 싱크/FEAT-04 제거/장바구니 명칭 반영)
...
- 렌더: `headerHTML(card)`/`skeleton(card)`/`updateBibleHeader`/`handleAction(card, act, actEl)`/`renderVersesInto(...)`/`renderMultiVersesInto(...)`(절별 대조 세로 교차)/**`renderSplitVersesInto(body, versions, chapDataArr, highlight)`**(v1.1.6 병렬 독서 좌우 컬럼 `.scol`+`.scol-head`)/`cardVersions(card)`(=`state.viewer`, 렌더·복사 공유)/`renderInterlinearInto`/`renderLexEntryInto`/`renderMorph` (※ FEAT-04 대조 pill·`"parallel"` 케이스 v1.1.6 제거, `renderNotesInto`/`wireNotes` 폐기)
